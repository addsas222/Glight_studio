...
- **新增保真自检** `docs/design/tools/check_design_fidelity.py`（换机可复跑，路径自定位，默认只报告）：
  ...
  实测：**文案 621 · 已落地 428 · 运行时·占位差异(允许) 138 · 真·缺口 55**。
- **55 条「真·缺口」的定性**：绝大多数不是代码缺失，而是**运行时值或设计占位**——
...
- ✅ **「把设计稿回写成可用代码」已验收**（2026-09-22 续作）：结论是「代码本就在，设计稿是它的
  ...
  - 新增 `docs/design/tools/check_design_fidelity.py`（换机可复跑，默认只报告）：逐个界面抽文案、
    去白色后在`整个前端源码`里查是否已落地。实测 **已落地 428 / 运行时·占位差异(允许) 138 / 真·缺口 55**。
  - 这 55 条里绝大多数是**运行时值或设计占位**：当前主题名「暗夜蓝调」、后端标签「深度来源：内置本地引擎」、
