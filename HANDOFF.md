...
- **新增保真自检** `docs/design/tools/check_design_fidelity.py`（换机可复跑，路径自定位，默认只报告）：
  ...
  实测：**文案 621 · 已落地 435 · 运行时·占位差异(允许) 136 · 真·缺口 50**。 **2026-09-22 收尾**（`is_runtime`+corpus 修正后复测）：**已落地 469 / 运行时·占位 150 / 真·缺口 2**，详见下条。
...
- ✅ **2026-09-22 收尾：真·缺口 50 → 2（并回写「光绘时间轴」）**：`check_design_fidelity.py` 仅按既定口径补全 `is_runtime()`——① 示例文件名（`.png/.jpg/…`）② `·` 组合（不再要求含 ASCII，`主光 · 点光` 等整体判运行时）③ `标签:值`（全角「：」，如 `当前主题：…`/`选中：…`）；`build_corpus()` 增计后端事实源 `core/theme.py` / `core/ai_backend.py` / `webui/api.py`（主题名与后端标签本就由后端定义、经 `/api/themes·/api/state` 供前端取用，属可用代码而非子串可比）。重跑同一批 621 条：**已落地 469 / 运行时·占位 150 / 真·缺口 2**。回写了唯一恰当的静态标签 `光绘时间轴`（framepaint.ts 底部时间轴头，复用 `small sec` 样式）；余下 2 条如实保留、不藏：`autolight.ts` `生成的光照节点会叠加到预览`（前缀在、尾串运行期拼装）、`starmap.ts` `主灯组`（运行时灯组名，随 `l.group`）。`选中：` **未强灌**——promode.ts 已有更明确的 `已选中「…」`、previs.ts 以高亮表达选择。换机复跑：`pytest` **99 passed / 1 skipped (211s)**、`tsc --noEmit` **exit 0**、`vite build` **exit 0**；注意**前端当前无 vitest 单测**（0 个 `*.test.ts`、无 vitest 依赖），前端可用性以 `tsc` + `vite build` 为准。
...
- ✅ **「把设计稿回写成可用代码」已验收**（2026-09-22 续作）：结论是「代码本就在，设计稿是它的
  ...
  - 新增 `docs/design/tools/check_design_fidelity.py`（换机可复跑，默认只报告）：逐个界面抽文案、
    去白色后在`整个前端源码`里查是否已落地。实测 **已落地 435 / 运行时·占位差异(允许) 136 / 真·缺口 50**。（**2026-09-22 收尾**修正校验器后复测：**已落地 469 / 运行时·占位 150 / 真·缺口 2**。）
